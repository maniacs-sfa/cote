...
module.exports = class TimeBalancedRequester extends Requester {
    constructor(advertisement, discoveryOptions) {
        ...
        setInterval(() => {
            ...
            for (index in this.responders) {
                for (id in this.responders[index]) {
        ...
        this.sock.on('socket close', (sock) => {
            if (sock.uuid) {
                let responder = this.responders[sock.uuid];
    ...
    send(...args) {
        ...
        if (len > 0 && _.size(this.responders) == len) { // have data for all sockets
            for (index in this.responders) {
                let item = this.responders[index];
                let totalTime = 0;
                let request;
                for (request in item) {
                    let time = item[request].time || 0;
                    totalTime += time;
                }
        ...
        let cb = (...args) => {
            let lastRequests = this.responders[index];
            ...
            let stats = { averageTime: (lastRequests && lastRequests.average) || 0 };
